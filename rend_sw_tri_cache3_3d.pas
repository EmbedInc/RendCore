{   Subroutine REND_SW_TRI_CACHE3_3D (V1, V2, V3, CA1, CA2, CA3, GNORM)
*
...
*       NONE.
*
*   10) The shading normals are always unitized.
*
*   11) The 3D to 3DW transform is not skewed, and has uniform scaling for
*       all axes.
}
module rend_sw_tri_cache3_3d;
define rend_sw_tri_cache3_3d;
%include 'rend_sw2.ins.pas';
%include 'rend_sw_tri_cache3_3d_d.ins.pas';
...
*   Fill in interpolants for vertex 1.
  ...
  if                                   {need to recompute final colors here ?}
    ...
    nv.x := flip_factor * (            {make 3D world space shading normal vector}
    ...
    ca1.colors_valid := true;          {cached colors are now up to date}
...
*   Fill in interpolants for vertex 2.
  ...
  if                                   {need to recompute final colors here ?}
    ...
    ca2.colors_valid := true;          {cached colors are now up to date}
    end;                               {done handling cached colors wern't valid}
{
*   Fill in interpolants for vertex 3.
  ...
  if                                   {need to recompute final colors here ?}
    ...
    ca3.colors_valid := true;          {cached colors are now up to date}
    end;                               {done handling cached colors wern't valid}
{
