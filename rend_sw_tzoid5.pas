...
*
*     1)  The enabled interpolants must be exactly red, green, and blue.
*
*     2)  RGB PIXFUN = INSERT.
*
*     3)  Interpolation mode either flat or linear for red, green, and blue.
*
*     4)  RGB interpolator clamping off, or values set to 0 and 255.
*
*     5)  RGB all write mask bits 1.
*
*     6)  Z buffering off, alpha buffering off, texture mapping off.
*
*   PRIM_DATA sw_read no
*   PRIM_DATA sw_write yes
}
module rend_sw_tzoid5;
define rend_sw_tzoid5;
%include 'rend_sw2.ins.pas';
%include 'rend_sw_tzoid5_d.ins.pas';
...
var
  ...
  red, grn, blu: rend_iterp_val_t;     {current interpolator values}
  red_inc, grn_inc, blu_inc:           {horizontal step interpolator increments}
    sys_int_machine_t;
  red_p, grn_p, blu_p: ^char;          {pointers to color pixel data}
  red_p_inc, grn_p_inc, blu_p_inc:     {horizontal offset for pixel data pntrs}
    sys_int_adr_t;
...
{
*   Later when stepping to the next scan line, we will assume that the RGB
